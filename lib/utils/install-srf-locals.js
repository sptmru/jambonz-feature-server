...
async function checkFreeswitchCallStatus(ms) {
  const fsStatusUrl = 'http://fs-status-api:8479/instanceCalls/' + ms.address;
  try {
    const response = await axios.get(fsStatusUrl);
    const callsQuantity =  response.data.callsQuantity;
    return callsQuantity < 1200 ? true : false;
  } catch (err) {
    logger.error(`checkFreeswitchCallStatus: error checking call status for ${ms.address}: ${err.message}`);
    return false;
  }
}
