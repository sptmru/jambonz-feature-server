...
function installSrfLocals(srf, logger) {
   ...
   */
  async function getFreeswitch() {
    const active = mediaservers.filter((mediaserver) => mediaserver.active);
    if (active.length === 0) return null;
    for (const ms of active) {
      logger.info(`List of Freeswitch instances includes ${ms.ms.address}`);
    }
    for (const ms of active) {
      if (await checkFreeswitchCallStatus(ms)) {
        logger.info(`getFreeswitch: using freeswitch at ${ms.ms.address}`);
        logger.info(ms.ms);
        return ms.ms;
      }
    }
    logger.info('getFreeswitch: no freeswitch available');
    return null;
  }

  async function checkFreeswitchCallStatus(ms) {
    const fsStatusUrl = `${FS_STATUS_API}/instanceCalls/${ms.address}`;
    try {
      const response = await axios.get(fsStatusUrl, { headers: {'Authorization': `Bearer ${FS_STATUS_API_KEY}` }});
      const callsQuantity =  response.data.callsQuantity;
      return callsQuantity < 1200 ? true : false;
    } catch (err) {
      logger.error(`checkFreeswitchCallStatus: error checking freeswitch call status: ${err.message}`);
      return false;
    }
  }

  async function incrementFreeswitchCalls(ms) {
    const fsStatusUrl = `${FS_STATUS_API}/instanceCalls/${ms.address}`;
    try {
      await axios.put(
        fsStatusUrl,
      ...
      logger.info(`incrementFreeswitchCalls: incremented freeswitch call status for ${ms.address}`);
    } catch (err) {
      // eslint-disable-next-line max-len
      logger.error(`incrementFreeswitchCalls: error incrementing freeswitch call status for ${ms.address}: ${err.message}`);
    }
  }

  async function decrementFreeswitchCalls(ms) {
    const fsStatusUrl = `${FS_STATUS_API}/instanceCalls/${ms.address}`;
    try {
      await axios.put(
        fsStatusUrl,
      ...
      logger.info(`incrementFreeswitchCalls: decremented freeswitch call status for ${ms.address}`);
    } catch (err) {
      // eslint-disable-next-line max-len
      logger.error(`incrementFreeswitchCalls: error decrementing freeswitch call status for ${ms.address}: ${err.message}`);
    }
  }
  ...
  srf.locals = {...srf.locals,
    ...
    lifecycleEmitter,
    getFreeswitch,
    incrementFreeswitchCalls,
    decrementFreeswitchCalls,
    stats: stats,
    writeAlerts,
    AlertType
  };
