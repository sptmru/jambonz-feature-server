...
class FsStatusApiWrapper {
  static async setFreeswitchCalls() {
    ...
    try {
      await axios.post(
        fsStatusUrl,
        { instanceId: `${K8S_POD_IP}`, callsQuantity: 0 }, { headers: {'Authorization': `Bearer ${FS_STATUS_API_KEY}` }}
      );
  ...
  static async deleteInstanceData(instanceId) {
    ...
    try {
      ...
      if (error.response) {
        logger.error(`deleteInstanceData: error status from the API: ${error.response.status}`);
        logger.error(`deleteInstanceData: error response from the API: ${error.response.data}`);
      }
    }
  }
}
