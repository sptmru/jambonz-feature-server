...
class SessionTracker extends Emitter {
  ...
  get logger() {
    if (!this._logger) {
      this._logger = this.srf.locals.logger;
    }
    return this._logger;
  }
  ...
  add(callSid, callSession) {
    ...
    void this.srf.locals.incrementFreeswitchCalls(callSession.hostname);
  ...
  remove(callSid) {
    ...
    void this.srf.locals.decrementFreeswitchCalls(session.hostname);
...
module.exports = (srf) => new SessionTracker(srf);
