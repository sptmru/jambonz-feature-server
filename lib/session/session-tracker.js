...
class SessionTracker extends Emitter {
  ...
  add(callSid, callSession) {
    ...
    const {srf} = '../..';
    const {incrementFreeswitchCalls} = srf.locals;
    void incrementFreeswitchCalls(callSession.hostname);
  ...
  remove(callSid) {
    ...
    const {srf} = '../..';
    const {decrementFreeswitchCalls} = srf.locals;
    void decrementFreeswitchCalls(session.hostname);
...
const {srf} = require('../..');
const singleton = new SessionTracker(srf);
