...
class SessionTracker extends Emitter {
  ...
  add(callSid, callSession) {
    ...
    try {
      const callSessionObject = JSON.parse(callSession);
      this.logger.info(callSessionObject);
      this.logger.info(`SessionTracker.remove: FS hostname after callSession parsing is ${callSessionObject.hostname}`);
    } catch (err) {
      this.logger.error(`Error parsing call session: ${err}`);
    }
    void FsStatusApiWrapper.incrementFreeswitchCalls(callSession.hostname);
  ...
  remove(callSid) {
    ...
    void FsStatusApiWrapper.decrementFreeswitchCalls(session.hostname);
    this.logger.info(`SessionTracker.remove: FS hostname is ${session.hostname}`);
