...
const {K8S_POD_NAME} = require('../config');
...
class SessionTracker extends Emitter {
  ...
  add(callSid, callSession) {
    ...
    this.logger.info(`SessionTracker.remove: FS address  is ${K8S_POD_NAME}`);
    void FsStatusApiWrapper.incrementFreeswitchCalls(K8S_POD_NAME);
  ...
  remove(callSid) {
    assert(callSid);
    this.logger.info(`SessionTracker.remove: FS hostname is ${K8S_POD_NAME}`);
    void FsStatusApiWrapper.decrementFreeswitchCalls(K8S_POD_NAME);
