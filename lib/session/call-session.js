...
const sessionTrackerFactory = require('./session-tracker');
...
class CallSession extends Emitter {
  ...
  constructor({logger, application, srf, tasks, callInfo, accountInfo, rootSpan, memberId, confName, confUuid}) {
    ...
    if (!this.isConfirmCallSession && !this.isSmsCallSession && !this.isAdultingCallSession) {
      this.sessionTracker.add(this.callSid, this);
 ...
 */
  ...
  async exec() {
    ...
    if (!this.isConfirmCallSession && !this.isSmsCallSession) this.sessionTracker.remove(this.callSid);
  }
  ...
  async _lccCallHook(opts) {
    ...
    if (childTasks) {
      ...
      this.sessionTracker.add(cs.callSid, cs);
    }
    if (tasks) {
