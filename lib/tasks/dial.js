...
class TaskDial extends Task {
  ...
  async kill(cs, reason) {
    ...
    if (this.callSid) this.sessionTracker.remove(this.callSid);
    if (this.listenTask) {
          ...
          try {
            ...
            if (tasks && tasks.length > 0) {
              ...
              if (isChild) this.redirect(cs, tasks);
                ...
                this.sessionTracker.add(adultingSession.callSid, adultingSession);
              }
            }
          } catch (err) {
 ...
 media already released, reconnect media server`);
  ...
  async _selectSingleDial(cs, sd) {
    ...
    this.sessionTracker.add(this.callSid, cs);
    this.dlg.on('destroy', () => {
      ...
      if (this.dlg) {
        ...
        this.sessionTracker.remove(this.callSid);
        if (this.timerMaxCallDuration) {
          clearTimeout(this.timerMaxCallDuration);
          this.timerMaxCallDuration = null;
        }
